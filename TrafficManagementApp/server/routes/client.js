...
class clientApplication {
    async submitTxn(
        ...
        try {
            ...
            if (txnType == "invokeTxn") {
            ...
            else if (txnType == "queryDelete") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
              else if (txnType == "invokeMatch") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
            else if (txnType == "invokeAccident") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
            else if (txnType == "queryAccident") {
                resultBytes = await contract.evaluateTransaction(txnName, ...args);
            }
            else if (txnType == "queryLawEnforcement") {
                resultBytes = await contract.evaluateTransaction(txnName, ...args);
            }
            else if (txnType == "queryAll") {
                resultBytes = await contract.evaluateTransaction(txnName, ...args);
            }
            else if (txnType == "invokeInsurance") {
                resultBytes = await contract.submitTransaction(txnName, ...args);
            }
                ...
                await contract.evaluateTransaction(txnName, {
                    arguments: [...args],
